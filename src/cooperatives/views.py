...
from core.mixins import RegionalAccessMixin, RegionalFormMixin, RoleRequiredMixin
...
def _scope_production_records_to_region(qs, user):
    """
    ProductionRecord has no direct region field: cherry deliveries reach a
    cooperative via station/member, generic harvests via farm. Check all three.
    """
    ...
    if not getattr(user, "region", None):
        return qs.none()
    return qs.filter(
        Q(farm__member__cooperative__region=user.region)
        | Q(station__cooperative__region=user.region)
        | Q(member__cooperative__region=user.region)
    )


def _get_cooperative_for_region(user, uuid):
    """Fetch a cooperative by uuid, scoped to the user's region (superusers see all)."""
    ...
    return get_object_or_404(qs, unique_id=uuid)
...
class CooperativeDetailView(RegionalAccessMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["members"] = paginator.get_page(self.request.GET.get("page"))
        ...
        context["sales_by_year"] = (
            self.object.sales.values("year").annotate(total_qty=Sum("quantity_kg")).order_by("year")
        )
        context["sales_by_destination"] = (
            self.object.sales.values("destination_country").annotate(total_qty=Sum("quantity_kg")).order_by("-total_qty")
        )
...
class CooperativeCreateView(RoleRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        if not getattr(self.request.user, "region", None):
        ...
        _log(self.request, AuditLog.ACTION_CREATE, self.object, description=f"Created cooperative: {self.object.name}")
...
class CooperativeUpdateView(RegionalAccessMixin, RoleRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        response = super().form_valid(form)
        _log(self.request, AuditLog.ACTION_UPDATE, self.object, description=f"Updated cooperative: {self.object.name}")
...
class CooperativeDeleteView(RegionalAccessMixin, RoleRequiredMixin, DeleteView):
    ...
    def form_valid(self, form):
        _log(self.request, AuditLog.ACTION_DELETE, self.object, description=f"Deleted cooperative: {self.object.name}")
...
class MemberListView(RegionalAccessMixin, ListView):
    ...
    region_field = "cooperative__region"
...
class MemberDetailView(RegionalAccessMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        prod_list = ProductionRecord.objects.filter(
            Q(farm__member=self.object) | Q(member=self.object)
        ).distinct().order_by("-harvest_date")
        paginator = Paginator(prod_list, 10)
        context["production_records"] = paginator.get_page(self.request.GET.get("page"))
...
class MemberCreateView(RegionalAccessMixin, RoleRequiredMixin, RegionalFormMixin, CreateView):
    ...
    def form_valid(self, form):
        response = super().form_valid(form)
        _log(self.request, AuditLog.ACTION_CREATE, self.object, description=f"Created member: {self.object}")
...
class MemberUpdateView(RegionalAccessMixin, RoleRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        response = super().form_valid(form)
        _log(self.request, AuditLog.ACTION_UPDATE, self.object, description=f"Updated member: {self.object}")
...
class MemberDeleteView(RegionalAccessMixin, RoleRequiredMixin, DeleteView):
    ...
    def form_valid(self, form):
        _log(self.request, AuditLog.ACTION_DELETE, self.object, description=f"Deleted member: {self.object}")
        return super().form_valid(form)


# ═════════════════════════════════════════════════════════════════════════════
# CHERRY DELIVERIES (ProductionRecord, record_type=cherry_delivery)
# ═════════════════════════════════════════════════════════════════════════════
...
class CherryDeliveryListView(RegionalAccessMixin, ListView):
    ...
    def get_queryset(self):
        ...
        ).select_related("station", "member")
        qs = _scope_production_records_to_region(qs, self.request.user)
...
class CherryDeliveryCreateView(RegionalAccessMixin, RoleRequiredMixin, RegionalFormMixin, CreateView):
    ...
    required_roles = ["manager", "admin", "regional_officer", "field_agent"]

    def get_form_kwargs(self):
        kwargs = super().get_form_kwargs()
        cooperative = None
        if getattr(self.request.user, "region", None):
    ...
    def form_valid(self, form):
        response = super().form_valid(form)
        _log(self.request, AuditLog.ACTION_CREATE, self.object, description=f"Recorded cherry delivery: {self.object}")

        # Best-effort offline sync queuing — failures here should not block the main flow.
        try:
            ...
            if device:
                PendingChange.objects.create(
                    ...
                    change_type=PendingChange.CHANGE_CREATE,
                    payload=model_to_dict(self.object),
                    local_timestamp=self.object.reception_date or self.object.purchase_date,
                )
        except Exception:
            pass
        return response
...
class CherryDeliveryDetailView(RegionalAccessMixin, DetailView):
    ...
    def get_queryset(self):
        return _scope_production_records_to_region(ProductionRecord.objects.all(), self.request.user)
...
class CherryDeliveryUpdateView(RegionalAccessMixin, RoleRequiredMixin, RegionalFormMixin, UpdateView):
    ...
    def form_valid(self, form):
        response = super().form_valid(form)
        _log(self.request, AuditLog.ACTION_UPDATE, self.object, description=f"Updated cherry delivery: {self.object}")
        return response
...
class CherryDeliveryDeleteView(RegionalAccessMixin, RoleRequiredMixin, DeleteView):
    ...
    def form_valid(self, form):
        _log(self.request, AuditLog.ACTION_DELETE, self.object, description=f"Deleted cherry delivery: {self.object}")
        return super().form_valid(form)


# ═════════════════════════════════════════════════════════════════════════════
# PRODUCTION RECORDS (generic harvest tracking)
# ═════════════════════════════════════════════════════════════════════════════
...
class ProductionRecordListView(RegionalAccessMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return _scope_production_records_to_region(qs, self.request.user)
...
class ProductionRecordCreateView(RegionalAccessMixin, RoleRequiredMixin, RegionalFormMixin, CreateView):
    ...
    required_roles = ["admin", "manager", "field_agent"]

    def form_valid(self, form):
        response = super().form_valid(form)
        _log(self.request, AuditLog.ACTION_CREATE, self.object, description=f"Recorded production: {self.object}")
        return response
...
class ProductionRecordUpdateView(RegionalAccessMixin, RoleRequiredMixin, RegionalFormMixin, UpdateView):
    ...
    def form_valid(self, form):
        response = super().form_valid(form)
        _log(self.request, AuditLog.ACTION_UPDATE, self.object, description=f"Updated production record: {self.object}")
        return response
...
class ProductionRecordDeleteView(RegionalAccessMixin, RoleRequiredMixin, DeleteView):
    ...
    def form_valid(self, form):
        _log(self.request, AuditLog.ACTION_DELETE, self.object, description=f"Deleted production record: {self.object}")
        return super().form_valid(form)
...
class FinancialRecordListView(RegionalAccessMixin, ListView):
    ...
    def get_queryset(self):
        return super().get_queryset().select_related("cooperative").order_by("-transaction_date")
...
class FinancialRecordCreateView(RegionalAccessMixin, RoleRequiredMixin, RegionalFormMixin, CreateView):
    ...
    def form_valid(self, form):
        response = super().form_valid(form)
        _log(self.request, AuditLog.ACTION_CREATE, self.object, description=f"Recorded financial record: {self.object}")
        return response
...
class FinancialRecordUpdateView(RegionalAccessMixin, RoleRequiredMixin, RegionalFormMixin, UpdateView):
    ...
    def form_valid(self, form):
        response = super().form_valid(form)
        _log(self.request, AuditLog.ACTION_UPDATE, self.object, description=f"Updated financial record: {self.object}")
        return response
...
class FinancialRecordDeleteView(RegionalAccessMixin, RoleRequiredMixin, DeleteView):
    ...
    def form_valid(self, form):
        _log(self.request, AuditLog.ACTION_DELETE, self.object, description=f"Deleted financial record: {self.object}")
        return super().form_valid(form)
...
class BuyerUpdateView(RegionalAccessMixin, RoleRequiredMixin, UpdateView):
    ...
    def get_queryset(self):
        # Buyers are a shared, global resource — not owned by any cooperative/region.
...
class CooperativeCertificateListView(RegionalAccessMixin, ListView):
    ...
    def get_cooperative(self):
        return _get_cooperative_for_region(self.request.user, self.kwargs.get("uuid"))
...
class CooperativeCertificateCreateView(RegionalAccessMixin, RoleRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        self.cooperative = _get_cooperative_for_region(request.user, kwargs.get("uuid"))
...
class WashingStationListView(RegionalAccessMixin, RoleRequiredMixin, ListView):
    ...
    paginate_by = 20
    required_roles = ["admin", "regional_officer", "manager", "field_agent"]
