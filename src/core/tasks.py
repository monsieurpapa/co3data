...
def compute_kpis_for_cooperative(self, cooperative_id: int, period_start: str, period_end: str):
    ...
    try:
        ...
        # Financial summary logic is now handled in SACCOFinancialSummary model
        # This task should be updated to use SACCOFinancialSummary if needed.
...
def run_data_quality_checks(self, cooperative_id: int = None):
    from cooperatives.models import Cooperative
    from analytics.models import DataValidationRule
    rules = DataValidationRule.objects.filter(is_active=True)
    coops = Cooperative.objects.all()
    if cooperative_id:
        coops = coops.filter(pk=cooperative_id)
    alerts_created = 0
    # Data quality checks for ProductionRecord were removed as models are gone.
    # New validation logic is handled via ValidationService.validate_record signal.
